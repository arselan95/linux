 ...
 * SDM volume 3, section 24.6.2
 */
struct capability_info procbased_secondary[25] =
{
 ...
 */
struct capability_info exit_control[11] =
{
 ...
 */
struct capability_info entry_control[9] =
{
...
detect_vmx_features(void)
{
	...
	if ( hi && (1 << 31))
	{
		...
		pr_info("\nSecondary procbased Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(procbased_secondary, 25, lo, hi);
	}
	else
	{
	...
	pr_info("\nExit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_control, 11, lo, hi);

	/* Entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("\nEntry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entry_control, 9, lo, hi);
}
